...
def handle_admin_callback(call: types.CallbackQuery) -> None:
    ...
    if action.startswith("process_withdraw:"):
        ...
        # Создаем чек для пользователя через Crypto Pay
        amount = Decimal(str(transaction["amount"]))
        transfer_amount = Decimal(transfer_amount_str)
        asset = transaction["asset"] if "asset" in transaction.keys() else settings.get("crypto_pay_asset", DEFAULT_SETTINGS["crypto_pay_asset"])
        ...
        # Создаем чек
        try:
            check_data = crypto_pay_client.create_check({
                "asset": asset,
                "amount": transfer_amount_str,
            })
        except CryptoPayError as exc:
            logger.warning("Failed to create check for withdraw %s: %s", transaction_id, exc)
            bot.answer_callback_query(call.id, f"Ошибка создания чека: {exc}", show_alert=True)
            return
        
        if not check_data:
            bot.answer_callback_query(call.id, "Ошибка создания чека", show_alert=True)
            return
        
        check_id = check_data.get("check_id")
        bot_check_url = check_data.get("bot_check_url")
        ...
        db.update_transaction(
            ...
            status="completed",
            external_id=str(check_id) if check_id is not None else None,
            comment=f"Manual withdraw - check #{check_id}",
        )
        ...
        user_text = (
            ...
            f"Сумма: <b>{format_money(amount)} $</b>\n"
            f"Активируйте чек для получения средств:"
        )
        ...
        try:
            ...
            logger.warning("Failed to send check to user %s: %s", user_id, exc)
        ...
        bot.answer_callback_query(call.id, "Заявка обработана, чек отправлен пользователю")
        logger.info(
            "Admin %s processed withdraw %s for user %s, check_id=%s",
            call.from_user.id,
            transaction_id,
            user_id,
            check_id,
